        ...
        try {
			...
			if(data.size()>0) {
				...
				for(GarageWorkOrder workOrder : data ) {
			         ...
			         if(sparePartList != null && sparePartList.size()>0) {
			        	 SparePartsSaveDetails spareParts = sparePartList.get(0);
				         ...
				         response.setVatRatePer(spareParts.getVatRate() != null 
				                ? spareParts.getVatRate().toString() : "0.00");
				         response.setVatRate(spareParts.getVatRatePercentage() != null 
				                ? spareParts.getVatRatePercentage().toString() : "0.00");
