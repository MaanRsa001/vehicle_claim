...
public class ExternalApiServiceImpl implements ExternalApiService {
	...
	public CommonResponse createFnol(SaveClaimRequest requestPayload) {
		...
		try {
			...
			// Optional<ClaimIntimationDetails> optional =
			// claimIntimationDetailsRepository.findByPolicyNo(requestPayload.getPolicyNo());
			Optional<ClaimIntimationDetails> optional = claimIntimationDetailsRepository
					.findByPolicyNoAndPoliceReportNo(requestPayload.getPolicyNo(), requestPayload.getPoliceReportNo());
		...
		try {
			...
			ExternalApiResponse externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
			...
			try {
				...
				Optional<ClaimIntimationDetails> optional = claimIntimationDetailsRepository
						.findByPolicyNoAndPoliceReportNo(requestPayload.getPolicyNo(),
								requestPayload.getPoliceReportNo());
				if (optional.isPresent()) {
			...
			if (externalApiResponse.isHasError()) {
				...
				for (ErrorDetail error : externalApiResponse.getData().getErrorDetailsList()) {
					errorList.add(new ErrorResponse("Azentio API " + externalApiUrlCreatefnol, error.getErrorField(),
			...
			response.setIsError(true);
			response.setErrors(Collections.singletonList(new ErrorResponse("100", "API Error", e.getMessage()))); // API
																													// Error
																													// error
		} finally {
	...
	public CommonResponse authenticateUser(LoginRequest request) {
		...
		try {
			...
			Map<String, String> formattedRequest = Map.of("username", request.getLoginId(), "password",
					request.getPassword());
	...
	public String authenticateUserCall() {
		...
		Map<String, String> formattedRequest = Map.of("username", username, "password", password);
	...
	public String saveOrUpdateClaimIntimation(SaveClaimRequest saveClaimRequestDTO) {
		try {
			...
			Optional<ClaimIntimationDetails> existingRecord = claimIntimationDetailsRepository
					.findById(new ClaimIntimationDetailsId(saveClaimRequestDTO.getPolicyNo(),
							saveClaimRequestDTO.getPoliceReportNo()));
			...
			claimIntimationDetails.setRequestGeneratedDateTime(
					saveClaimRequestDTO.getRequestMetaData().getRequestGeneratedDateTime());

			claimIntimationDetails
					.setConsumerTrackingId(saveClaimRequestDTO.getRequestMetaData().getConsumerTrackingID());
			...
			logger.severe("Error saving/updating data for policyNo: " + saveClaimRequestDTO.getPolicyNo() + " - "
					+ e.getMessage());
	...
	public CommonResponse getClaimByPolicy(GetClaimRequest req) {
		...
		try {
			...
			Optional<ClaimIntimationDetails> dataOptional = claimIntimationDetailsRepository
					.findByPolicyNoAndPoliceReportNo(req.getPolicyNo(), req.getPoliceReportNo());
	...
	public CommonResponse getAllClaims() {
		CommonResponse response = new CommonResponse();
		try {
			List<ClaimIntimationDetails> dataList = claimIntimationDetailsRepository
					.findAllByOrderByRequestGeneratedDateTimeDesc();
			List<ClaimIntimationResponse> responseList = dataList.stream().map(this::mapToClaimIntimationResponse)
...
//	        response.setIntimatedDate(data.getIntimatedDate());
	...
	// Example of mapping nested objects (you need to complete as per your
	// structure)
...
//                }
	...
	private ClaimListRequestDTO mapToClaimListingDTO(ClaimListRequest requestPayload) {
		...
		try {
			...
			if (requestMetaData != null) {
				try {
					...
					dtoRequestMetaData.setRequestGeneratedDateTime(requestMetaData.getRequestGeneratedDateTime() != null
		...
		return new ClaimListRequestDTO(requestPayload.getLobCode(), requestPayload.getProdCode(),
				requestPayload.getPolicyNumber(), requestPayload.getClaimNotificationNumber(),
				requestPayload.getCreatedBy(), claimNotificationFromDate, claimNotificationToDate, claimLossFromDate,
				claimLossToDate, dtoRequestMetaData);
	}
	...
	public CommonResponse saveSpareParts(SaveSparePartsDTO request) {
		...
		try {
			...
			GarageWorkOrder workOrder = garageWorkOrderRepo.findByClaimNoAndQuotationNo(request.getClaimNo(),
					request.getQuotationNo());
			LoginMaster loginMaster = loginRepo.findByLoginId(workOrder.getGarageId());
			List<DamageSectionDetails> damageDetails = damageSectionDetailsRepo
					.findByClaimNoAndQuotationNo(request.getClaimNo(), request.getQuotationNo());
			SparePartsSaveDetails partsSaveDetails = SparePartsSaveDetailsRepo
					.findByClaimNoAndGarageCode(request.getClaimNo(), loginMaster.getCoreAppCode());
			if (workOrder == null || damageDetails.isEmpty()) {
				response.setMessage("No data found for the provided claim or work order number");
				response.setIsError(true);
				response.setErrors(Collections.singletonList(new ErrorResponse("404", "Data Not Found",
						"Data not found for the provided claim or work order number")));
				return response;
			} else if ("Y".equalsIgnoreCase(partsSaveDetails.getSavedStatus())
					|| "ESB".equalsIgnoreCase(partsSaveDetails.getSavedStatus())) {
				// response.setMessage("No data found for the provided claim or work order
				// number");
				...
				// response.setErrors(Collections.singletonList(new ErrorResponse("404", "Data
				// Not Found", "Data not found for the provided claim or work order number")));
				return response;
			}

			// Map entities to request DTO
			// SaveSparePartsRequest dto = mapToSaveSparePartsRequest(workOrder,
			// damageDetails);
...
//			request.setClaimNo(partsSaveDetails.getClaimNo());
			...
			for (DamageSectionDetails detail : damageDetails) {
				...
				String directionCode = dropDownServiceImpl.getItemCodeByItemValue(detail.getDamageDirection(),
						"DAMAGE_DIRECTION");
				...
				if ("REPLACE".equalsIgnoreCase(detail.getRepairReplace())) {
					damageRequest
							.setUnitPrice(detail.getGaragePrice() != null ? detail.getGaragePrice().toString() : "");
					...
					BigDecimal replacementCharge = detail.getReplaceCost() != null ? detail.getReplaceCost()
							: BigDecimal.ZERO;
					...
					damageRequest.setDeductiblePer(detail.getLabourCostDeductPercentage() != null
							? detail.getLabourCostDeductPercentage().toString()
							: "0");
					damageRequest.setDeductibleAmount(
							detail.getLabourCostDeduct() != null ? detail.getLabourCostDeduct().toString() : "0");
					// damageRequest.setBeforeDeduction(replacementCharge!=null ?
					// replacementCharge.toString():"0");
					damageRequest.setBeforeDeduction("");
					BigDecimal dedudct = detail.getLabourCostDeduct() != null ? detail.getLabourCostDeduct()
							: BigDecimal.ZERO;
					total = replacementCharge.subtract(dedudct);
					damageRequest.setTotal("");
				}

				damageRequest.setReplacementCharge(
						detail.getReplaceCost() != null ? detail.getReplaceCost().toString() : "0");
			...
			metaData.setRequestGeneratedDateTime(isoDateFormat.format(new Date()));
			// metaData.setConsumerTrackingID(UUID.randomUUID().toString()); // example,
...
//			request.setReplacementCost(partsSaveDetails.getReplacementCost() != null ? partsSaveDetails.getReplacementCost().toString() : "0");
			...
			request.setReplacementCostDeductible(partsSaveDetails.getReplacementCostDeductible() != null
					? partsSaveDetails.getReplacementCostDeductible().toString()
					: "0");
			request.setSparePartDepreciation(partsSaveDetails.getSparePartDepreciation() != null
					? partsSaveDetails.getSparePartDepreciation().toString()
					: "0");
			request.setDiscountonSpareParts(partsSaveDetails.getDiscountOnSpareParts() != null
					? partsSaveDetails.getDiscountOnSpareParts().toString()
					: "0");
...
//			request.setRepairLabour(partsSaveDetails.getRepairLabour() != null ? partsSaveDetails.getRepairLabour().toString() : "0");
			...
			request.setRepairLabourDeductible(partsSaveDetails.getRepairLabourDeductible() != null
					? partsSaveDetails.getRepairLabourDeductible().toString()
					: "0");
			request.setRepairLabourDiscountAmount(partsSaveDetails.getRepairLabourDiscountAmount() != null
					? partsSaveDetails.getRepairLabourDiscountAmount().toString()
					: "0");
			request.setTotalAmountRepairLabour("");
			// request.setTotalAmountRepairLabour(partsSaveDetails.getTotalAmountRepairLabour()
			// != null ? partsSaveDetails.getTotalAmountRepairLabour().toString() : "0");
			request.setNetAmount(
					partsSaveDetails.getNetAmount() != null ? partsSaveDetails.getNetAmount().toString() : "0");
			request.setUnkownAccidentDeduction(partsSaveDetails.getUnknownAccidentDeduction() != null
					? partsSaveDetails.getUnknownAccidentDeduction().toString()
					: "0");
			request.setAmounttobeRecovered(partsSaveDetails.getAmountToBeRecovered() != null
					? partsSaveDetails.getAmountToBeRecovered().toString()
					: "0");
//			request.setTotalafterDeductions(partsSaveDetails.getTotalAfterDeductions() != null ? partsSaveDetails.getTotalAfterDeductions().toString() : "0");
			...
			request.setVatRatePer(
					partsSaveDetails.getVatRatePercentage() != null ? partsSaveDetails.getVatRatePercentage().toString()
							: "0");
			request.setVatRate(partsSaveDetails.getVatRate() != null ? partsSaveDetails.getVatRate().toString() : "0");
			request.setVatAmount(
					partsSaveDetails.getVatAmount() != null ? partsSaveDetails.getVatAmount().toString() : "0");
			request.setTotalWithVAT(
					partsSaveDetails.getTotalWithVat() != null ? partsSaveDetails.getTotalWithVat().toString() : "0");
	...
	private SaveSparePartsRequest mapToSaveSparePartsRequest(GarageWorkOrder workOrder,
			List<DamageSectionDetails> damageDetails) {
		SaveSparePartsRequest request = new SaveSparePartsRequest();
...
//			    damageRequest.setTotal(detail.getTotPrice());
	...
	/**
	 * Configures SSL Trust Managers to trust all certificates. This should only be
	 * used in a development or testing environment.
	 */
		...
		try {
			...
			if (spareSavedList != null && spareSavedList.size() > 0) {
				...
				for (SparePartsSaveDetails spareSaved : spareSavedList) {
					...
					response.setSparepartsDealerId(
							Optional.ofNullable(spareSaved.getSparePartsDealer()).map(String::valueOf).orElse(""));

					// Format dates inline
					response.setWorkOrderDate(
							spareSaved.getWorkOrderDate() != null ? dateFormat.format(spareSaved.getWorkOrderDate())
									: "");
					response.setDeliveryDate(
							spareSaved.getDeliveryDate() != null ? dateFormat.format(spareSaved.getDeliveryDate())
									: "");
					...
					response.setRepairLabour(
							spareSaved.getRepairLabour() != null ? spareSaved.getRepairLabour().toString() : "0.00");
					...
					response.setUnknownAccidentDeduction(spareSaved.getUnknownAccidentDeduction() != null
							...
							: "0.00");
					response.setAmountToBeRecovered(
							spareSaved.getAmountToBeRecovered() != null ? spareSaved.getAmountToBeRecovered().toString()
									: "0.00");
					...
					response.setVatRatePer(
							spareSaved.getVatRatePercentage() != null ? spareSaved.getVatRatePercentage().toString()
									: "0.00");
					response.setVatRate(spareSaved.getVatRate() != null ? spareSaved.getVatRate().toString() : "0.00");
					response.setVatAmount(
							spareSaved.getVatAmount() != null ? spareSaved.getVatAmount().toString() : "0.00");
					response.setTotalWithVAT(
							spareSaved.getTotalWithVat() != null ? spareSaved.getTotalWithVat().toString() : "0.00");
	...
	public CommonResponse getSavedGarageSpareParts(SaveSparePartsDTO requestPayload) {
		...
		try {
			...
			// List<SparePartsSaveDetails> spareSavedList =
			// SparePartsSaveDetailsRepo.findByGarageCode(loginMaster.getCoreAppCode());
			List<SparePartsSaveDetails> spareSavedList = SparePartsSaveDetailsRepo
					.findByGarageCodeOrderByEntryDateDesc(loginMaster.getCoreAppCode());
	...
	public CommonResponse getGarageClaimList(ExternalVehicleGarageViewRequest request) {
		...
		try {
			...
			// Parse response into ExternalApiResponse object
			GarageClaimListResponseDTO externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					GarageClaimListResponseDTO.class);
	...
	public CommonResponse checkClaimStatus(CheckClaimStatusRequest request) {
		...
		try {
			...
			// Parse response into ExternalApiResponse object
			ClaimsDetailsResponseDto externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					ClaimsDetailsResponseDto.class);
				...
				for (ClaimDetails CD : claimsDetailsList) {
					...
					try {
						surveyorId = loginList.stream()
								...
								.map(LoginMaster::getLoginId).findFirst().orElse("");
	...
	private void saveClaimStatusToTable(ClaimDetails claim) {
		try {
			...
			// data.setUpdatedAt(new Date()); // You can update this based on your
			// requirements
	...
	public CommonResponse listClaimantCoverages(ClaimentCoverageRequest request) {
		...
		try {
			...
			// Parse response into ExternalApiResponse object
			ClaimentCoverageResponseDTO externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					ClaimentCoverageResponseDTO.class);
	...
	public CommonResponse lpoSyncView(LpoApprovalSyncRequest requestPayload) {
		...
		try {
			...
			// Parse response into ExternalApiResponse object
			LpoApprovalSyncResponse externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					LpoApprovalSyncResponse.class);
	...
	private List<GarageSectionDetailsSaveReq> mapToGroupedDamageDetails(LpoApprovalSyncResponse externalApiResponse,
			Map<String, String> damageDirectionMap) {
	...
	public CommonResponse getGarageWorkOrder(GetGarageWorkOrderRequest requestPayload) {
		...
		try {
			...
			ResponseEntity<String> apiResponse = restTemplate.postForEntity(transactionLog.getEndpoint(), entity,
					String.class);

			// Parse API response
			WorkOrderPendingResponseDto externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					WorkOrderPendingResponseDto.class);
			...
			List<String> fileNos = apiData.stream().map(WorkOrderDetailResponseDto::getFileNo)
					.collect(Collectors.toList());
			...
			for (SparePartsSaveDetails spareSaved : sparePartsList) {
				...
				if (spareSaved != null) {
					...
					quoteResponse.setSavedStatus(
							"ESB".equalsIgnoreCase(spareSaved.getSavedStatus()) ? "WST" : spareSaved.getSavedStatus());
					quoteResponse.setSparepartsDealerId(
							Optional.ofNullable(spareSaved.getSparePartsDealer()).map(String::valueOf).orElse(""));
					...
					// Format dates inline
					quoteResponse.setWorkOrderDate(
							spareSaved.getWorkOrderDate() != null ? dateFormat.format(spareSaved.getWorkOrderDate())
									: "");
					...
					try {
						...
						System.out.println("There is a error in formating the delivery date :" + spareSaved.getClaimNo()
								+ " " + expDeliveryDateStr);
					}
	...
	public CommonResponse getGarageWorkOrderV0(GetGarageWorkOrderRequest requestPayload) {
		...
		try {
			...
			Set<InsuredVehicleInfoId> insuredIds = apiData
					.stream().map(insured -> new InsuredVehicleInfoId(requestPayload.getCompanyid(),
							insured.getPolicyNo(), insured.getClaimNo(), requestPayload.getGarageid()))
			...
			List<InsuredVehicleInfo> insuredInfoList = apiData.stream()
					.filter(insured -> existingIds.contains(new InsuredVehicleInfoId(requestPayload.getCompanyid(),
							insured.getPolicyNo(), insured.getFileNo(), requestPayload.getGarageid())))
					.map(insured -> {
	...
	public CommonResponse getGarageSettlement(GarageSettlementListRequest requestPayload) {
		...
		try {
			...
			// Parse API response
			GarageSettlementResponseDto externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					GarageSettlementResponseDto.class);
			...
			// Collect file numbers for batch fetching
			List<String> fileNos = apiData.stream().map(GarageSettlementListResponseDto::getFileNo)
					.collect(Collectors.toList());
			List<SparePartsSaveDetails> sparePartsList = new ArrayList<>();
...
//	                quoteResponse.setDeliveryDate(sparePartsMap.get(spareSaved.getClaimNo()).getDeliveryDate() != null ? dateFormat.format(sparePartsMap.get(spareSaved.getClaimNo()).getDeliveryDate()) : "");
					...
					if (!"ESB".equalsIgnoreCase(spareSaved.getSavedStatus())) {
						if ("Completed"
								.equalsIgnoreCase(sparePartsMap.get(spareSaved.getClaimNo()).getPaymentStatus())) {
	...
	private String uplodeDocument(GetGarageWorkOrderRequest req) {
		try {
			...
			for (VcDocumentUploadDetails doc : docList) {
				DocumentUploadDetailsReqRes docRes = DocumentUploadDetailsReqRes.builder().claimNo(doc.getClaimNo())
						.documentRef(String.valueOf(doc.getDocumentRef()))
						.docTypeId(doc.getDocTypeId() != null ? String.valueOf(doc.getDocTypeId()) : null)
						.docDesc(doc.getDescription()).companyId(String.valueOf(doc.getCompanyId()))
						.filePathName(doc.getFilePathName()).fileName(doc.getFileName()).uploadType(doc.getUploadType())
						.commonFilePath(doc.getCommonFilePath()).errorRes(doc.getErrorRes())
						.imgUrl(doc.getFilePathName()).uploadedBy(doc.getUploadedBy()).fileType(doc.getFileType())
						.build();
	...
	private void PushDocument(GetGarageWorkOrderRequest req, FileUploadRequestDto dto, String filePath) {
		...
		try {
			...
			// Send request to external API
			ResponseEntity<String> apiResponse = restTemplate.exchange(log.getEndpoint(), HttpMethod.POST,
...
//	            dto.setAmndVersionId(req.getAmndVersionId());
	...
	public CommonResponse getUploadFileList(DownloadDocumentRequest req) {
		...
		try {
			...
			// Parse response into ExternalApiResponse object
			UploadedDocumentListResponseDto externalApiResponse = objectMapper.readValue(apiResponse.getBody(),
					UploadedDocumentListResponseDto.class);
